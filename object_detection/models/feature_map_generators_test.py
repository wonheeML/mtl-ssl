...
class ConcatFeatureMapsTest(tf.test.TestCase):

  def test_get_expected_feature_map_shapes(self):
    image_features = {
        'conv24': tf.random_uniform([4, 15, 20, 1024], dtype=tf.float32),
        'conv16': tf.random_uniform([4, 30, 40, 512], dtype=tf.float32),
        'conv10': tf.random_uniform([4, 60, 80, 256], dtype=tf.float32)
    }
    feature_map = feature_map_generators.concatenate_feature_maps(
        image_features=image_features,
        layer_names=['conv10', 'conv16', 'conv24'])

    expected_feature_map_shape = (4, 15, 20, 7168)

    init_op = tf.global_variables_initializer()
    with self.test_session() as sess:
      sess.run(init_op)
      out_feature_map = sess.run(feature_map)
      self.assertEqual(out_feature_map.shape, expected_feature_map_shape)
