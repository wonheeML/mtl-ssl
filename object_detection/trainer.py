...
def _create_input_queue(batch_size_per_clone, create_tensor_dict_fn,
                        ...
                        prefetch_queue_capacity, data_augmentation_options, ignore_options=None):
...
def _get_inputs(input_queue, num_classes):
  ...
  def extract_images_and_targets(read_data):
    ...
    crowd_gt = read_data.get(fields.InputDataFields.groundtruth_is_crowd)
    if crowd_gt.get_shape() is not classes_gt.get_shape():
      crowd_gt = tf.zeros_like(classes_gt, dtype=tf.bool)
    ...
    return image, location_gt, ignore_gt, crowd_gt, classes_gt, masks_gt
...
def _create_losses(input_queue, create_model_fn):
  ...
  (images, groundtruth_boxes_list, groundtruth_ignore_list,
   groundtruth_crowd_list, groundtruth_classes_list, groundtruth_masks_list
  ...
  detection_model.provide_groundtruth(groundtruth_boxes_list,
                                      ...
                                      groundtruth_crowd_list,
                                      groundtruth_masks_list)
  prediction_dict = detection_model.predict(images)

  losses_dict = detection_model.loss(prediction_dict)
  for loss_name, loss_tensor in losses_dict.iteritems():
    tf.losses.add_loss(tf.identity(loss_tensor, name='Loss/' + loss_name))


def train(create_tensor_dict_fn, create_model_fn, train_config, master, task,
          num_clones, worker_replicas, clone_on_cpu, ps_tasks, worker_job_name,
          is_chief, train_dir, num_examples):
  ...
  with tf.Graph().as_default():
    ...
    if train_config.fine_tune_checkpoint:
      if train_config.HasField('restore_backbone_only'):
        var_map = detection_model.restore_map(
            from_detection_checkpoint=train_config.from_detection_checkpoint,
            restore_backbone_only=train_config.restore_backbone_only)
      else:
        var_map = detection_model.restore_map(
            from_detection_checkpoint=train_config.from_detection_checkpoint)
