...
def preprocess(tensor_dict, preprocess_options, func_arg_map=None):
  ...
  # changes the box width(height) if it is 0
  prevent_box_size_zero(tensor_dict)

  return tensor_dict


def prevent_box_size_zero(tensor_dict):
  """ Prevent the box size from being zero
  """

  if fields.InputDataFields.groundtruth_boxes not in tensor_dict:
    pass

  gt_boxes = tensor_dict[fields.InputDataFields.groundtruth_boxes]
  gt_lt = gt_boxes[:, 0:2]
  gt_rb = gt_boxes[:, 2:4]

  epsilon = 1e-8
  gt_lt = tf.clip_by_value(gt_lt, clip_value_min=0.0, clip_value_max=1.0 - epsilon)
  gt_rb = tf.clip_by_value(gt_rb, clip_value_min=epsilon, clip_value_max=1.0)

  tensor_dict[fields.InputDataFields.groundtruth_boxes] = tf.concat([gt_lt, gt_rb], axis=1)


def make_ignore_list(tensor_dict, ignore_options):
  """Preprocess ignore list.

  Add ignore information based on various ignore options.

  Args:
    tensor_dict: (see preprocess definition)
    ignore_options:

  Returns:
    tensor_dict: added ignore information(bool)
  ...
  ignore = tf.zeros_like(
    tensor_dict[fields.InputDataFields.groundtruth_classes], dtype=tf.bool)

  if ignore_options.use_ignore is False:
    tensor_dict[fields.InputDataFields.groundtruth_ignore] = ignore
  else:
    if fields.InputDataFields.groundtruth_height_pixel in tensor_dict:
      gt_height = tensor_dict[fields.InputDataFields.groundtruth_height_pixel]
      min_height = ignore_options.min_height
      ignore = tf.logical_or(ignore, tf.less(gt_height, min_height))

    if fields.InputDataFields.groundtruth_width_pixel in tensor_dict:
      gt_width= tensor_dict[fields.InputDataFields.groundtruth_width_pixel]
      min_width = ignore_options.min_width
      ignore = tf.logical_or(ignore, tf.less(gt_width, min_width))

    if fields.InputDataFields.groundtruth_label_types in tensor_dict:
      gt_label_type = \
          tensor_dict[fields.InputDataFields.groundtruth_label_types]
      for label_type_to_ignore in ignore_options.label_type_to_ignore:
        ignore = tf.logical_or(
          ignore, tf.equal(gt_label_type, label_type_to_ignore))
