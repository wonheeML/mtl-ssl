...
class BoxList(object):
  ...
  def get_lefttop_coordinates_and_sizes(self, scope=None):
    ...
    Returns:
      a list of 4 1-D tensors [ymin, xmin, height, width].
    """
  ...
  def boolean_mask(self, mask):
    tensor_box = self.get()
    tensor_box_masked = tf.boolean_mask(tensor_box, mask)
    boxes_masked = BoxList(tensor_box_masked)
    for field in self.get_extra_fields():
      tensor = self.get_field(field)
      tensor_masked = tf.boolean_mask(tensor, mask)
      boxes_masked.add_field(field, tensor_masked)
    return boxes_masked