...
class PreprocessorTest(tf.test.TestCase):
  ...
  def testTrainingIgnore(self):
    object_height_pixel = tf.constant([49, 50, 100, 100, 100, 100], dtype=tf.float32)
    object_width_pixel = tf.constant([50, 49, 100, 100, 100, 100], dtype=tf.float32)
    source_id = tf.constant([1, 2, 3, 4, 5, 6], dtype=tf.int32)
    label_types = tf.constant(['appropriate', 'test', 'test', 'incorrect', 'test', 'test'], dtype=tf.string)

    tensor_dict = {
        fields.InputDataFields.groundtruth_height_pixel: object_height_pixel,
        fields.InputDataFields.groundtruth_width_pixel: object_width_pixel,
        fields.InputDataFields.source_id: source_id,
        fields.InputDataFields.groundtruth_label_types: label_types
    }

    class IgnoreOptionsTemp:
      def __init__(self):
        self.use_ignore = False
        self.min_height = 0
        self.min_width = 0
        self.label_type_to_ignore = ['incorrect']

    ignore_options = IgnoreOptionsTemp()
    preprocessor.make_ignore_list(tensor_dict, ignore_options)

    with self.test_session() as sess:
      tensor_val = sess.run(tensor_dict)
      self.assertAllClose(tensor_val[fields.InputDataFields.groundtruth_ignore],
