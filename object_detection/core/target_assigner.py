...
class TargetAssigner(object):
  ...
  def assign(self, anchors, groundtruth_boxes, groundtruth_labels=None,
    ...
    Args:
      ...
      predicted_boxes: a BoxList representing N predicted boxes
    ...
    def apply_ignore(cls_weights, match, match_ignore):
      # negative & ignore matched -> cls_weight = 0

      ignore_matched_indicator = match_ignore.matched_column_indicator()
      gt_unmatched_indicator = match.unmatched_column_indicator()
      ignore_indicator = tf.logical_and(ignore_matched_indicator, gt_unmatched_indicator)

      zeros_weight = tf.zeros(tf.shape(cls_weights))
    ...
    standard_boxes = anchors
    if predicted_boxes is not None:
      if not isinstance(predicted_boxes, box_list.BoxList):
        raise ValueError('predicted_boxes must be an BoxList')
      standard_boxes = predicted_boxes
    ...
    with tf.control_dependencies([shape_assert]):
      gt_labels, gt_boxes, crowd_boxes, ignore_boxes = self.categorize_crowd_ignore(groundtruth_labels, groundtruth_boxes)

      #match_quality_matrix = self._similarity_calc.compare(gt_boxes, anchors)
  ...
  def categorize_crowd_ignore(self, groundtruth_total_labels, groundtruth_total_boxes):
    ...
    """
    def get_bool_indicator(boxes, field):
      if boxes.num_boxes() is not 0 and boxes.has_field(field):
        data = boxes.get_field(field)
        shape = data.get_shape()
        if len(shape) > 0 and shape[0] is boxes.num_boxes():
          return data
        else:
          return tf.zeros([boxes.num_boxes()], dtype=tf.bool)
      else:
        return tf.zeros([boxes.num_boxes()], dtype=tf.bool)

    ignore_indicator = get_bool_indicator(groundtruth_total_boxes, fields.BoxListFields.ignore)
    crowd_indicator = get_bool_indicator(groundtruth_total_boxes, fields.BoxListFields.crowd)
    gt_indicator = tf.logical_not(tf.logical_or(ignore_indicator, crowd_indicator))

    ignore_boxes = groundtruth_total_boxes.boolean_mask(ignore_indicator)
    crowd_boxes = groundtruth_total_boxes.boolean_mask(crowd_indicator)
    gt_boxes = groundtruth_total_boxes.boolean_mask(gt_indicator)
    gt_labels = tf.boolean_mask(groundtruth_total_labels, gt_indicator)

    return gt_labels, gt_boxes, crowd_boxes, ignore_boxes
...
def batch_assign_targets(target_assigner,
  ...
  Args:
    ...
    gt_class_targets_batch: a list of tensors with length batch_size, where
      ...
      gt_box_batch.
    dt_box_batch: a list of BoxList objects with length batch_size
      representing predicted boxes for each image in the batch
  ...
  if dt_box_batch is not None:
    if not all(
        isinstance(dt_boxes, box_list.BoxList) for dt_boxes in dt_box_batch):
      raise ValueError('dt_box_batch must be a BoxList or list of BoxLists.')
  else:
    dt_box_batch = len(gt_box_batch) * [None]
